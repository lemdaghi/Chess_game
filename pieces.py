    ...
    def get_moves(self, board, simulate=False):
        ...
        for dx, dy in directions:
            ...
            if 0 <= nx < 8 and 0 <= ny < 8:
                ...
                if piece is None or piece.color != self.color:  # Empty Case or Ennemy Case
                    # Test if it's an illegal move
                    board.grid[y][x] = None  
                    board.grid[ny][nx] = self 
                    self.position = (nx, ny)  

                    if not simulate and not ChessRules.is_in_check(board, self.color):  # Verify if the king is not checked in the new position
                        moves.append((nx, ny)) 

                    # Restore the initial state
                    board.grid[ny][nx] = piece
                    board.grid[y][x] = self
